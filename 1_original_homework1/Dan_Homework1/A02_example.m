% ----------------------------------------
% SINGLE INTERFACE WITH V2>V1 (refraction)
% ----------------------------------------
...
for kx=1:Nx,
  ...
  for kz=1:Nz,
    ...
    if z>z_interface,
      model.vel(kz,kx)=2000; % Z meters depth bigger, so it is (below)
    else
      model.vel(kz,kx)=800; %  Z meters depth smaller, so its up 
    end
...
%// 40 receivers along z = 20 m, 
%// from x = 120…900 m every 20 m 
%// (offsets 20…800 m relative to the source).
...
% ----------------------------------------
% 2. Source parameters
%//Single source at (x,z) = (100, 20) m, Ricker wavelet, 
%//f₀ = 25 Hz, emission time t0 = 0.04 s 发射时间, unit amplitude.
...
% ----------------------------------------
% 3. Simulation and graphic parameters in structure simul
%// Simulation length 1.2 s; absorbing boundaries on to suppress wrap-around; 开启吸收边界以抑制环绕
%// plotting every 10 time steps; grayscale colormap.
simul.borderAlg=1;
simul.timeMax=1.2;
...
%// Outputs. recfield contains the pressure vs time at each receiver; 
%// then seisplot2 displays wiggle+fill traces versus offset.
