...
model.z   = 0:1:1000;     % vertical z axis sampling 
% I changed the z vector grid  coordinations to improve the size of model
...
% I changed the constant velocity, decrease it.

% ----------------------------------------
% 2. Source parameters (in the center of the model)

source.x    = 500;   %[model.x(end)/2 ];
source.z    = 500;   %[model.z(end)/2 ]; 
% I changed the ricker wavelet source position to the central of the
% receivers circle.
source.f0   = [40 ];
source.t0   = [0.04  ];
source.amp  = [1 ];
source.type = [1];    % 1: ricker, 2: sinusoidal  at f0
% I still use the 1 ricker


% optional receivers in (recx, recz)
% the program round their position on the nearest velocity grid
Nr     = 150;   % nr of receivers
% I added the Number of receiviers.
