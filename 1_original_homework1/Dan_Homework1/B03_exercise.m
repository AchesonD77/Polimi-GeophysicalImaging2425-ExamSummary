...
clear all
%I built a 2-D acoustic model with a high-velocity channel (1000 m/s) 
% between 20 and 40 m depth.
% The layers above and below are very slow (10 m/s), 
% so the channel behaves like an acoustic
% ----------------------------------------
% 1. Model parameters

model.x   = 0:0.25:300;     % horizontal x axis sampling
model.z   = 0:0.25:60;     % vertical   z axis sampling
...
source.x    = 30;   %[model.x(end)/2 ];
source.z    = 30;   %[model.z(end)/2 ]; 
source.f0   = [40 ];
source.t0   = [0.04  ];
source.amp  = [1 ];
...
model.recz  = 15:15:45;
Nreceivers  = numel(model.recz);

model.recx  = ones(1,Nreceivers) * model.x(end)-10;
model.dtrec = 0.004;
