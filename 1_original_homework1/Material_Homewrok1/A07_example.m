% ----------------------------------------
% Refraction again (with large borders to keep noise away)
% ----------------------------------------
%
% -------------------------------------------------------------------------
...
model.z   = 0:1:800;     % vertical   z axis sampling
...
model.recx  = [210:10:(model.x(end)-150)];
Nreceivers  = numel(model.recx);

model.recz  = ones(1,Nreceivers) + 300;
model.dtrec = 0.004;
...
simul.cmap='bone';   % gray, cool, hot, parula, hsv
...
colour = ''; % trace colour, default is black
clip   = .85; % clipping of amplitudes (if <1); default no clipping
